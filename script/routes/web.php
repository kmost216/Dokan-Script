...
Route::group(
	...
	function($domain) {
		...
		Route::group(
			['prefix' => 'cron_job', 'namespace' => 'Admin'],
			function() {
		...
		Route::group(
			['as' => 'merchant.', 'prefix' => 'merchant', 'middleware' => ['auth']],
			function() {
		...
		Route::get(
			...
			function() {
				return response(
					file_get_contents(base_path('sitemap.xml')),
					200,
					[
...
Route::group(
	['domain' => '{subdomain}.' . env('APP_PROTOCOLESS_URL')],
	function() {
		$url = Request::getHost();
		$full_url = url('/');
...
Route::group(
	['as' => 'author.', 'prefix' => 'author', 'namespace' => 'Admin', 'middleware' => ['auth', 'author']],
	function() {
...
Route::group(
	['as' => 'seller.', 'prefix' => 'seller', 'namespace' => 'Seller', 'middleware' => ['auth', 'seller']],
	function() {
		Route::get('dashboard', 'DashboardController@dashboard')->name('dashboard');
